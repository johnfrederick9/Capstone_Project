...
try {
    ...
    $rao_cocont_id = isset($_POST['rao_cocont_id']) ? intval($_POST['rao_cocont_id']) : '';

    // Validate rao_id
    if ($rao_cocont_id <= 0) {
        echo json_encode(['status' => 'false', 'error' => 'Invalid rao_cocont_id']);
        exit();
    }

    // Prepare and execute SQL query
    $sql_fetch_attr_name = "SELECT rao_cocont_att_id, attribute_name FROM tb_rao_cocont_attributes WHERE rao_cocont_id = ? AND isDisplayed = 1";
    $stmt_fetch_attr_name = mysqli_prepare($con, $sql_fetch_attr_name);
    mysqli_stmt_bind_param($stmt_fetch_attr_name, "i", $rao_cocont_id);
    ...
    $attribute_names = [];
    $attribute_ids = [];  // To store the rao_cocont_att_id
    
    // Fetch all rows and collect attribute names and ids
    while ($row = mysqli_fetch_assoc($result)) {
        $attribute_names[] = $row['attribute_name'];
        $attribute_ids[] = $row['rao_cocont_att_id'];  // Store the rao_cocont_att_id
    }
    
    // Close the statement
    mysqli_stmt_close($stmt_fetch_attr_name);

    $sql_fetch_rao_cocont = "SELECT chairman, period_covered, brgy_captain FROM tb_rao_cocont WHERE rao_cocont_id = ? AND isDisplayed = 1 ";
    $stmt_fetch_rao_cocont = mysqli_prepare($con, $sql_fetch_rao_cocont);
    mysqli_stmt_bind_param($stmt_fetch_rao_cocont, "i", $rao_cocont_id);

    if (!mysqli_stmt_execute($stmt_fetch_rao_cocont)) {
        throw new Exception("Error executing fetch rao_cocont query: " . mysqli_error($con));
    }

    // Fetch the result
    $result_fetch_rao_cocont = mysqli_stmt_get_result($stmt_fetch_rao_cocont);

    if ($row = mysqli_fetch_assoc($result_fetch_rao_cocont)) {
        $chairman = $row['chairman'];
        $period_covered = $row['period_covered'];
        $brgy_captain = $row['brgy_captain'];
    } else {
        throw new Exception("No data found for rao_cocont_id: " . $rao_cocont_id . " in tb_rao_cocont");
    }

    // Close the statement to free resources
    mysqli_stmt_close($stmt_fetch_rao_cocont);

    /////////////////////////////// AP

    $rao_cocont_ap = []; // Initialize the array for storing fetched rao_cocont_ap data
    $rao_cocont_ap_data = []; // Initialize the array for storing fetched rao_cocont_ap_data

    // Fetch data from tb_rao_cocont_ap
    $sql_fetch_rao_cocont_ap = "SELECT * FROM tb_rao_cocont_ap WHERE rao_cocont_id = ?";
    $stmt_fetch_rao_cocont_ap = mysqli_prepare($con, $sql_fetch_rao_cocont_ap);
    mysqli_stmt_bind_param($stmt_fetch_rao_cocont_ap, "i", $rao_cocont_id);
    if (!mysqli_stmt_execute($stmt_fetch_rao_cocont_ap)) {
        throw new Exception("Error executing fetch rao_cocont_ap query: " . mysqli_error($con));
    }
    $rao_cocont_ap_result = mysqli_stmt_get_result($stmt_fetch_rao_cocont_ap);

    // Fetch rows into $rao_cocont_ap
    while ($row = $rao_cocont_ap_result->fetch_assoc()) {
        $rao_cocont_ap[] = $row; // Append each row to the array
    }

    mysqli_stmt_close($stmt_fetch_rao_cocont_ap);

    // Fetch data from tb_rao_cocont_ap_data for each rao_cocont_ap_id
    foreach ($rao_cocont_ap as $apItem) {
        $rao_cocont_ap_id = $apItem['rao_cocont_ap_id']; // Get rao_cocont_ap_id from the row

        $sql_fetch_rao_cocont_ap_data = "SELECT * FROM tb_rao_cocont_ap_data WHERE rao_cocont_ap_id = ? AND isDisplayed = 1 ORDER BY rao_cocont_att_id ASC";
        $stmt_fetch_rao_cocont_ap_data = mysqli_prepare($con, $sql_fetch_rao_cocont_ap_data);
        mysqli_stmt_bind_param($stmt_fetch_rao_cocont_ap_data, "i", $rao_cocont_ap_id);
        if (!mysqli_stmt_execute($stmt_fetch_rao_cocont_ap_data)) {
            throw new Exception("Error executing fetch rao_cocont_ap_data query: " . mysqli_error($con));
        }
        $rao_cocont_ap_result_data = mysqli_stmt_get_result($stmt_fetch_rao_cocont_ap_data);

        // Fetch rows into $rao_cocont_ap_data
        while ($row = $rao_cocont_ap_result_data->fetch_assoc()) {
            $rao_cocont_ap_data[] = $row; // Append each row to the array
        }

        mysqli_stmt_close($stmt_fetch_rao_cocont_ap_data);
    }

    //Fetch from tb_rao_cocont_ap_totals
    $rao_cocont_ap_BF_totals = [];
    $rao_cocont_ap_TA_totals = [];
    $rao_cocont_ap_grand_totals = [];

    // Fetch data from tb_rao_cocont_ap_totals
    $sql_fetch_rao_cocont_ap_totals = "SELECT * FROM tb_rao_cocont_ap_totals WHERE rao_cocont_id = ? AND isDisplayed = 1 ORDER BY rao_cocont_att_id ASC";
    $stmt_fetch_rao_cocont_ap_totals = mysqli_prepare($con, $sql_fetch_rao_cocont_ap_totals);
    mysqli_stmt_bind_param($stmt_fetch_rao_cocont_ap_totals, "i", $rao_cocont_id);

    if (!mysqli_stmt_execute($stmt_fetch_rao_cocont_ap_totals)) {
        throw new Exception("Error executing fetch rao_cocont_ap query: " . mysqli_error($con));
    }

    $rao_cocont_ap_totals_result = mysqli_stmt_get_result($stmt_fetch_rao_cocont_ap_totals);

    // Fetch rows and separate by total_type
    while ($row = $rao_cocont_ap_totals_result->fetch_assoc()) {
        if ($row['total_type'] == 'BF') {
            $rao_cocont_ap_BF_totals[] = $row;
        } elseif ($row['total_type'] == 'TA') {
            $rao_cocont_ap_TA_totals[] = $row;
        }else {
            $rao_cocont_ap_grand_totals[] = $row;
        }
    }

    // Close the statement
    mysqli_stmt_close($stmt_fetch_rao_cocont_ap_totals);


    //////////////////////////////////// OB

    $rao_cocont_ob = []; // Initialize the array for storing fetched rao_cocont_ob data
    $rao_cocont_ob_data = []; // Initialize the array for storing fetched rao_cocont_ob_data

    // Fetch data from tb_rao_cocont_ob
    $sql_fetch_rao_cocont_ob = "SELECT * FROM tb_rao_cocont_ob WHERE rao_cocont_id = ?";
    $stmt_fetch_rao_cocont_ob = mysqli_prepare($con, $sql_fetch_rao_cocont_ob);
    mysqli_stmt_bind_param($stmt_fetch_rao_cocont_ob, "i", $rao_cocont_id);
    if (!mysqli_stmt_execute($stmt_fetch_rao_cocont_ob)) {
        throw new Exception("Error executing fetch rao_cocont_ob query: " . mysqli_error($con));
    }
    $rao_cocont_ob_result = mysqli_stmt_get_result($stmt_fetch_rao_cocont_ob);

    // Fetch rows into $rao_cocont_ob
    while ($row = $rao_cocont_ob_result->fetch_assoc()) {
        $rao_cocont_ob[] = $row; // Append each row to the array
    }

    mysqli_stmt_close($stmt_fetch_rao_cocont_ob);

    // Fetch data from tb_rao_cocont_ob_data for each rao_cocont_ob_id
    foreach ($rao_cocont_ob as $obItem) {
        $rao_cocont_ob_id = $obItem['rao_cocont_ob_id']; // Get rao_cocont_ob_id from the row

        $sql_fetch_rao_cocont_ob_data = "SELECT * FROM tb_rao_cocont_ob_data WHERE rao_cocont_ob_id = ? AND isDisplayed = 1 ORDER BY rao_cocont_att_id ASC";
        $stmt_fetch_rao_cocont_ob_data = mysqli_prepare($con, $sql_fetch_rao_cocont_ob_data);
        mysqli_stmt_bind_param($stmt_fetch_rao_cocont_ob_data, "i", $rao_cocont_ob_id);
        if (!mysqli_stmt_execute($stmt_fetch_rao_cocont_ob_data)) {
            throw new Exception("Error executing fetch rao_cocont_ob_data query: " . mysqli_error($con));
        }
        $rao_cocont_ob_result_data = mysqli_stmt_get_result($stmt_fetch_rao_cocont_ob_data);

        // Fetch rows into $rao_cocont_ob_data
        while ($row = $rao_cocont_ob_result_data->fetch_assoc()) {
            $rao_cocont_ob_data[] = $row; // Append each row to the array
        }

        mysqli_stmt_close($stmt_fetch_rao_cocont_ob_data);
    }

    //Fetch from tb_rao_cocont_ob_totals
    $rao_cocont_ob_TO_totals = [];
    $rao_cocont_ob_OB_totals = [];
    $rao_cocont_ob_AB_totals = [];
    $rao_cocont_ob_grand_totals = [];

    // Fetch data from tb_rao_cocont_ob_totals
    $sql_fetch_rao_cocont_ob_totals = "SELECT * FROM tb_rao_cocont_ob_totals WHERE rao_cocont_id = ? AND isDisplayed = 1 ORDER BY rao_cocont_att_id ASC";
    $stmt_fetch_rao_cocont_ob_totals = mysqli_prepare($con, $sql_fetch_rao_cocont_ob_totals);
    mysqli_stmt_bind_param($stmt_fetch_rao_cocont_ob_totals, "i", $rao_cocont_id);

    if (!mysqli_stmt_execute($stmt_fetch_rao_cocont_ob_totals)) {
        throw new Exception("Error executing fetch rao_cocont_ob query: " . mysqli_error($con));
    }

    $rao_cocont_ob_totals_result = mysqli_stmt_get_result($stmt_fetch_rao_cocont_ob_totals);

    // Fetch rows and separate by total_type
    while ($row = $rao_cocont_ob_totals_result->fetch_assoc()) {
        if ($row['total_type'] == 'TO') {
            $rao_cocont_ob_TO_totals[] = $row;
        } elseif ($row['total_type'] == 'OB') {
            $rao_cocont_ob_OB_totals[] = $row;
        }elseif($row['total_type'] == 'AB'){
            $rao_cocont_ob_AB_totals[] = $row;
        }else{
            $rao_cocont_ob_grand_totals[] = $row;
        }
    }

    // Close the statement
    mysqli_stmt_close($stmt_fetch_rao_cocont_ob_totals);
    ...
    $response = [
        ...
        'rao_cocont_ap' => $rao_cocont_ap,
        'rao_cocont_ap_data' => $rao_cocont_ap_data,
        'rao_cocont_ap_BF_totals' =>  $rao_cocont_ap_BF_totals,
        'rao_cocont_ap_TA_totals' =>  $rao_cocont_ap_TA_totals,
        'rao_cocont_ob' => $rao_cocont_ob,
        'rao_cocont_ob_data' => $rao_cocont_ob_data,
        'rao_cocont_ob_TO_totals' => $rao_cocont_ob_TO_totals,
        'rao_cocont_ob_OB_totals'  =>$rao_cocont_ob_OB_totals,
        'rao_cocont_ob_AB_totals' => $rao_cocont_ob_AB_totals,
        'rao_cocont_ap_grand_totals'=>$rao_cocont_ap_grand_totals,
        'rao_cocont_ob_grand_totals'=>$rao_cocont_ob_grand_totals
...
<div class="rao-container">
            ...
            <div class="rao-header">
                <h1>Report of Appropriations and Obligations (CO-CONT)</h1>
     ...
     function formatPeriodCovered(dateString) {
        ...
        console.log(json);
                ...
                if (json.status === 'true') {
                    ...
                    console.log("totals:",json.rao_cocont_ap_grand_totals)
                    console.log("totals:",json.rao_cocont_ob_grand_totals)
...
if (json.rao_cocont_ap && Array.isArray(json.rao_cocont_ap) && json.rao_cocont_ap.length > 0) {
    json.rao_cocont_ap.forEach(function (apItem) {
        ...
        <tr class="ap-data-row">
            ...
            <td class="hidden">${apItem.rao_cocont_ap_id || ''}</td>  <!-- Directly display rao_cocont_ap_id -->
        `;
...
if (json.rao_cocont_ob && Array.isArray(json.rao_cocont_ob) && json.rao_cocont_ob.length > 0) {
    json.rao_cocont_ob.forEach(function (obItem) {
        ...
        <tr class="ob-data-row">
            ...
            <td class="hidden">${obItem.rao_cocont_ob_id || ''}</td>  <!-- Directly display rao_cocont_ob_id -->
        `;
...
rowConfigs.ap.forEach(config => {
    ...
    if (config.includeTotal) {
        ...
        const totalArray = json[`rao_cocont_ap_grand_totals`]; // Adjust based on your JSON structure
    ...
    attributeList.forEach((attr, index) => {
        ...
        if (attrId) {
            ...
            const totalsArray = json[`rao_cocont_ap_${config.identifier}_totals`];
            const totalEntry = totalsArray?.find(
                apTotal => apTotal.rao_cocont_att_id == attrId
            );
...
rowConfigs.ob.forEach(config => {
    ...
    if (config.includeTotal) {
        ...
        // Retrieve the corresponding total entry based on total type and identifier
        const totalArray = json[`rao_cocont_ob_grand_totals`]; // Adjust based on your JSON structure
    ...
    attributeList.forEach((attr, index) => {
        ...
        if (attrId) {
            ...
            // Access the appropriate totals array directly from the response object
            const totalsArray = json[`rao_cocont_ob_${config.identifier}_totals`];
            const totalEntry = totalsArray?.find(
                obTotal => obTotal.rao_cocont_att_id === attrId
            );
            console.log("Total Array: ",totalsArray);
            console.log("Total Entry: ",totalEntry);
