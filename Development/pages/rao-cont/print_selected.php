...
try {
    ...
    $rao_cont_id = isset($_POST['rao_cont_id']) ? intval($_POST['rao_cont_id']) : '';
    ...
    $response = [
        ...
        'rao_cont_ap_grand_totals'=>$rao_cont_ap_grand_totals,
        'rao_cont_ob_grand_totals'=>$rao_cont_ob_grand_totals
     ...
     function formatPeriodCovered(dateString) {
        ...
        console.log(json);
                ...
                if (json.status === 'true') {
                    ...
                    console.log("totals:",json.rao_cont_ap_grand_totals)
                    console.log("totals:",json.rao_cont_ob_grand_totals)
...
if (json.rao_cont_ap && Array.isArray(json.rao_cont_ap) && json.rao_cont_ap.length > 0) {
    json.rao_cont_ap.forEach(function (apItem) {
        ...
        <tr class="ap-data-row">
            ...
            <td class="hidden">${apItem.rao_cont_ap_id || ''}</td>  <!-- Directly display rao_cont_ap_id -->
        `;
...
if (json.rao_cont_ob && Array.isArray(json.rao_cont_ob) && json.rao_cont_ob.length > 0) {
    json.rao_cont_ob.forEach(function (obItem) {
        ...
        <tr class="ob-data-row">
            ...
            <td class="hidden">${obItem.rao_cont_ob_id || ''}</td>  <!-- Directly display rao_cont_ob_id -->
        `;
...
rowConfigs.ap.forEach(config => {
    ...
    if (config.includeTotal) {
        ...
        const totalArray = json[`rao_cont_ap_grand_totals`]; // Adjust based on your JSON structure
    ...
    attributeList.forEach((attr, index) => {
        ...
        if (attrId) {
            ...
            const totalsArray = json[`rao_cont_ap_${config.identifier}_totals`];
            const totalEntry = totalsArray?.find(
                apTotal => apTotal.rao_cont_att_id === attrId
            );
...
rowConfigs.ob.forEach(config => {
    ...
    if (config.includeTotal) {
        ...
        // Retrieve the corresponding total entry based on total type and identifier
        const totalArray = json[`rao_cont_ob_grand_totals`]; // Adjust based on your JSON structure
    ...
    attributeList.forEach((attr, index) => {
        ...
        if (attrId) {
            ...
            // Access the appropriate totals array directly from the response object
            const totalsArray = json[`rao_cont_ob_${config.identifier}_totals`];
            const totalEntry = totalsArray?.find(
                obTotal => obTotal.rao_cont_att_id === attrId
            );
