...
try {
    ...
    $rao_cont_id = $_POST['rao_cont_id'];  // The rao_cont_id from the form
    $period_covered = $_POST['period_covered'];
    $chairman = $_POST['chairman'];
    $brgy_captain = $_POST['brgy_captain'];

    // Update the tb_rao_cont with the chairman, period_covered, and brgy_captain
    ...
    if (!mysqli_stmt_execute($stmt)) {
        throw new Exception("Error updating tb_rao_cont: " . mysqli_error($con));
    }
    ...
    if (isset($_POST['ap_data']) && is_array($_POST['ap_data'])) {
        foreach ($_POST['ap_data'] as $apRow) {
            ...
            $sql = "INSERT INTO tb_rao_cont_ap (rao_cont_id, ap_ref_date, ap_ref_no, ap_particulars, ap_totals) 
                    VALUES (?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($con, $sql);
            mysqli_stmt_bind_param($stmt, 'isssd', $rao_cont_id, $date, $reference_no, $particulars, $total);
            if (!mysqli_stmt_execute($stmt)) {
            ...
            if (isset($apRow['attributes']) && is_array($apRow['attributes'])) {
                foreach ($apRow['attributes'] as $attributeId => $attributeValue) {
                    ...
                    // Insert the dynamic attribute into tb_rao_cont_ob_data
                    $sql_insert_ap_data = "INSERT INTO tb_rao_cont_ap_data (rao_cont_ap_id, rao_cont_att_id, attribute_name, attribute_value, isDisplayed)                                 
                                           VALUES (?, ?, ?, ?, 1)";
                    $stmt_insert_ap_data = mysqli_prepare($con, $sql_insert_ap_data);
                    mysqli_stmt_bind_param($stmt_insert_ap_data, 'iiss', $rao_cont_ap_id, $attributeId, $attributeName, $attributeValue);
    ...
    if (isset($_POST['ob_data']) && is_array($_POST['ob_data'])) {
        foreach ($_POST['ob_data'] as $obRow) {
            ...
            $sql = "INSERT INTO tb_rao_cont_ob (rao_cont_id, ob_ref_date, ob_ref_no, ob_particulars, ob_totals) 
                    VALUES (?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($con, $sql);
            mysqli_stmt_bind_param($stmt, 'isssd', $rao_cont_id, $date, $reference_no, $particulars, $total);
            if (!mysqli_stmt_execute($stmt)) {
            ...
            if (isset($obRow['attributes']) && is_array($obRow['attributes'])) {
                foreach ($obRow['attributes'] as $attributeId => $attributeValue) {
                    ...
                    $sql_insert_ob_data = "INSERT INTO tb_rao_cont_ob_data (rao_cont_ob_id, rao_cont_att_id, attribute_name, attribute_value, isDisplayed) 
                                           VALUES (?, ?, ?, ?,1)";
                    $stmt_insert_ob_data = mysqli_prepare($con, $sql_insert_ob_data);
                    mysqli_stmt_bind_param($stmt_insert_ob_data, 'iiss', $rao_cont_ob_id, $attributeId, $attributeName, $attributeValue);
    ...
    foreach ($_POST['ap_totals'] as $identifier => $values) {
        if (is_array($values)) {
            foreach ($values as $attributeId => $totalValue) {
                ...
                $sql_insert_ap_total = "INSERT INTO tb_rao_cont_ap_totals (rao_cont_id, total_type, rao_cont_att_id, attribute_name, attribute_value, isDisplayed) 
                                      VALUES (?, ?, ?, ?, ?, 1)";
    ...
    foreach ($_POST['ob_totals'] as $identifier => $values) {
        if (is_array($values)) {
            foreach ($values as $attributeId => $totalValue) {
                ...
                $sql_insert_ob_total = "INSERT INTO tb_rao_cont_ob_totals (rao_cont_id, total_type, rao_cont_att_id, attribute_name, attribute_value, isDisplayed) 
                                      VALUES (?, ?, ?, ?, ?, 1)";
