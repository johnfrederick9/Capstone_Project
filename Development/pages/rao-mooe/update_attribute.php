...
$rao_mooe_att_ids = json_decode($_POST["rao_mooe_att_id"], true);
...
try {
    ...
    // Step 1: Get existing attribute IDs for the given rao_mooe_id
    $sql_existing_attributes = "SELECT rao_mooe_att_id, attribute_name FROM tb_rao_mooe_attributes 
        WHERE rao_mooe_id = ? AND isDisplayed = 1";
    $stmt_existing = mysqli_prepare($con, $sql_existing_attributes);
    mysqli_stmt_bind_param($stmt_existing, "i", $rao_mooe_id);
    ...
    while ($row = mysqli_fetch_assoc($result_existing)) {
        $existing_att_ids[] = $row['rao_mooe_att_id'];
        $existing_att_names[$row['rao_mooe_att_id']] = $row['attribute_name'];
    }
    ...
    $att_ids_to_remove = array_values(array_diff($existing_att_ids, $rao_mooe_att_ids)); // Ensure it's an indexed array
    ...
    if (!empty($att_ids_to_remove)) {
        // Delete from tb_rao_mooe_attributes
        $sql_delete_attr = "UPDATE tb_rao_mooe_attributes SET isDisplayed = 0 WHERE rao_mooe_att_id = ? AND rao_mooe_id = ?";
        foreach($att_ids_to_remove as $att_id){
            $stmt_delete_attr = mysqli_prepare($con, $sql_delete_attr);
            mysqli_stmt_bind_param($stmt_delete_attr, "ii", $att_id, $rao_mooe_id);
            if (!mysqli_stmt_execute($stmt_delete_attr)) {
                throw new Exception("Failed to delete attribute from tb_rao_mooe_attributes: " . mysqli_stmt_error($stmt_delete_attr));
            }
            mysqli_stmt_close($stmt_delete_attr);

            // Delete from related tables (AP data and OB data)
            $sql_delete_ap = "UPDATE tb_rao_mooe_ap_data SET isDisplayed = 0 WHERE rao_mooe_att_id = ?";
            ...
            $sql_delete_ob = "UPDATE tb_rao_mooe_ob_data SET isDisplayed = 0 WHERE rao_mooe_att_id = ?";
            ...
            $sql_delete_ap_totals = "UPDATE tb_rao_mooe_ap_totals SET isDisplayed = 0 WHERE rao_mooe_att_id = ?";
            ...
            $sql_delete_ob_totals = "UPDATE tb_rao_mooe_ob_totals SET isDisplayed = 0 WHERE rao_mooe_att_id = ?";
    ...
    foreach ($attribute_names as $index => $name) {
        $att_id = $rao_mooe_att_ids[$index];

        // // Step 4a: Validate if the provided attribute ID is valid (exists in the database for the given rao_mooe_id)
        ...
        if (!empty($att_id)) {
         // Update existing attribute
            $sql_update = "UPDATE tb_rao_mooe_attributes 
            SET attribute_name = ? 
            WHERE rao_mooe_att_id = ? AND rao_mooe_id = ? AND isDisplayed = 1";
            $stmt = mysqli_prepare($con, $sql_update);
            mysqli_stmt_bind_param($stmt, "sii", $name, $att_id, $rao_mooe_id);
            ...
            // Update AP data
            $sql_update_ap_data = "UPDATE tb_rao_mooe_ap_data SET attribute_name = ? WHERE rao_mooe_att_id = ? AND isDisplayed = 1";
            ...
            // Update OB data
            $sql_update_ob_data = "UPDATE tb_rao_mooe_ob_data SET attribute_name = ? WHERE rao_mooe_att_id = ? AND isDisplayed = 1";
            ...
            $sql_update_ap_totals = "UPDATE tb_rao_mooe_ap_totals SET attribute_name = ? WHERE rao_mooe_att_id = ? AND isDisplayed = 1";
            ...
            $sql_update_ob_totals = "UPDATE tb_rao_mooe_ob_totals SET attribute_name = ? WHERE rao_mooe_att_id = ? AND isDisplayed = 1";
            ...
            $sql_check_existing = "SELECT attribute_name FROM tb_rao_mooe_attributes 
                    WHERE attribute_name = ? AND rao_mooe_id = ? AND isDisplayed = 0";
            $stmt_check_existing = mysqli_prepare($con, $sql_check_existing);
            mysqli_stmt_bind_param($stmt_check_existing, "si", $name, $rao_mooe_id);
            ...
            $result_check_existing = mysqli_stmt_get_result($stmt_check_existing);
             ...
             if (mysqli_num_rows($result_check_existing) > 0) {
                $sql_restore = "UPDATE tb_rao_mooe_attributes 
                                SET isDisplayed = 1
                                WHERE attribute_name = ? AND rao_mooe_id = ?";
                $stmt_restore = mysqli_prepare($con, $sql_restore);
                mysqli_stmt_bind_param($stmt_restore, "si", $name, $rao_mooe_id);
                ...
                $sql_fetch_id = "SELECT rao_mooe_att_id FROM tb_rao_mooe_attributes 
                                 WHERE attribute_name = ? AND rao_mooe_id = ?";
                $stmt_fetch_id = mysqli_prepare($con, $sql_fetch_id);
                mysqli_stmt_bind_param($stmt_fetch_id, "si", $name, $rao_mooe_id);
                ...
                //Restore Ap Data
                $sql_restore_rao_mooe_ap_data = "UPDATE tb_rao_mooe_ap_data
                                SET isDisplayed = 1
                                WHERE rao_mooe_att_id = ?";
                $stmt_restore_rao_mooe_ap_data = mysqli_prepare($con, $sql_restore_rao_mooe_ap_data);
                mysqli_stmt_bind_param($stmt_restore_rao_mooe_ap_data, "i", $restored_id);
            
                if (!mysqli_stmt_execute($stmt_restore_rao_mooe_ap_data)) {
                    throw new Exception("Failed to restore attribute: " . mysqli_stmt_error($stmt_restore_rao_mooe_ap_data));
                }
                mysqli_stmt_close($stmt_restore_rao_mooe_ap_data);


                //Restore Ob Data
                $sql_restore_rao_mooe_ob_data = "UPDATE tb_rao_mooe_ob_data
                                SET isDisplayed = 1
                                WHERE rao_mooe_att_id = ?";
                $stmt_restore_rao_mooe_ob_data = mysqli_prepare($con, $sql_restore_rao_mooe_ob_data);
                mysqli_stmt_bind_param($stmt_restore_rao_mooe_ob_data, "i", $restored_id);

                if (!mysqli_stmt_execute($stmt_restore_rao_mooe_ob_data)) {
                    throw new Exception("Failed to restore attribute: " . mysqli_stmt_error($stmt_restore_rao_mooe_ob_data));
                }
                mysqli_stmt_close($stmt_restore_rao_mooe_ob_data);

                  //Restore Ap Totals
                  $sql_restore_rao_mooe_ap_totals = "UPDATE tb_rao_mooe_ap_totals
                                SET isDisplayed = 1
                                WHERE rao_mooe_att_id = ? AND rao_mooe_id = ?";
                $stmt_restore_rao_mooe_ap_totals = mysqli_prepare($con, $sql_restore_rao_mooe_ap_totals);
                mysqli_stmt_bind_param($stmt_restore_rao_mooe_ap_totals, "ii", $restored_id, $rao_mooe_id);

                if (!mysqli_stmt_execute($stmt_restore_rao_mooe_ap_totals)) {
                    throw new Exception("Failed to restore attribute: " . mysqli_stmt_error($stmt_restore_rao_mooe_ap_totals));
                }
                mysqli_stmt_close($stmt_restore_rao_mooe_ap_totals);

                  //Restore Ob Totals
                  $sql_restore_rao_mooe_ob_totals = "UPDATE tb_rao_mooe_ob_totals
                                SET isDisplayed = 1
                                WHERE rao_mooe_att_id = ? AND rao_mooe_id = ?";
                $stmt_restore_rao_mooe_ob_totals = mysqli_prepare($con, $sql_restore_rao_mooe_ob_totals);
                mysqli_stmt_bind_param($stmt_restore_rao_mooe_ob_totals, "ii", $restored_id, $rao_mooe_id);

                if (!mysqli_stmt_execute($stmt_restore_rao_mooe_ob_totals)) {
                    throw new Exception("Failed to restore attribute: " . mysqli_stmt_error($stmt_restore_rao_mooe_ob_totals));
                }
                mysqli_stmt_close($stmt_restore_rao_mooe_ob_totals);

            
                $restored_attributes[] = array(
                    'rao_mooe_att_id' => $restored_id,
                    'attribute_name' => $name
                );
                ...
                $sql_insert = "INSERT INTO tb_rao_mooe_attributes 
                            (rao_mooe_id, attribute_name, isDisplayed) 
                            VALUES (?, ?, 1)";
                $stmt = mysqli_prepare($con, $sql_insert);
                mysqli_stmt_bind_param($stmt, "is", $rao_mooe_id, $name);
                ...
                $inserted_attributes[] = array(
                    'rao_mooe_att_id' => $new_att_id,
                    'attribute_name' => $name
                );

                // Insert into AP data for existing AP entries
                $sql_ap_data = "INSERT INTO tb_rao_mooe_ap_data (rao_mooe_ap_id, rao_mooe_att_id, attribute_name, attribute_value, isDisplayed)
                            SELECT rao_mooe_ap_id, ?, ?, 0 , 1
                            FROM tb_rao_mooe_ap 
                            WHERE rao_mooe_id = ?";
                $stmt_ap_data = mysqli_prepare($con, $sql_ap_data);
                mysqli_stmt_bind_param($stmt_ap_data, "isi", $new_att_id, $name, $rao_mooe_id);
                ...
                $sql_ob_data = "INSERT INTO tb_rao_mooe_ob_data (rao_mooe_ob_id, rao_mooe_att_id, attribute_name, attribute_value, isDisplayed)
                            SELECT rao_mooe_ob_id, ?, ?, 0  , 1
                            FROM tb_rao_mooe_ob 
                            WHERE rao_mooe_id = ?";
                $stmt_ob_data = mysqli_prepare($con, $sql_ob_data);
                mysqli_stmt_bind_param($stmt_ob_data, "isi", $new_att_id, $name, $rao_mooe_id);
                ...
                foreach ($total_types_ap as $type) {
                    $sql_ap_totals = "INSERT INTO tb_rao_mooe_ap_totals 
                    ...
                    mysqli_stmt_bind_param($stmt_ap_totals, "isis", $rao_mooe_id, $type, $new_att_id, $name);
                ...
                foreach ($total_types_ob as $type) {
                    $sql_ob_totals = "INSERT INTO tb_rao_mooe_ob_totals 
                                    (rao_mooe_id, total_type, rao_mooe_att_id, attribute_name, attribute_value, isDisplayed)
                                    VALUES (?, ?, ?, ?, 0 , 1)";
                    $stmt_ob_totals = mysqli_prepare($con, $sql_ob_totals);
                    mysqli_stmt_bind_param($stmt_ob_totals, "isis", $rao_mooe_id, $type, $new_att_id, $name);
