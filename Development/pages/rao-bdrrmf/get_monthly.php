...
try {
    ...
    if($task == "update"){
         ...
         $sql_date_check = "SELECT COUNT(*) as month_count FROM tb_rao_bd WHERE DATE_FORMAT(period_covered, '%Y-%m') = ? AND isDisplayed = 1 AND rao_bd_id != ?";
        ...
        $sql_date_check = "SELECT COUNT(*) as month_count FROM tb_rao_bd WHERE DATE_FORMAT(period_covered, '%Y-%m') = ? AND isDisplayed = 1";
        $stmt_date_check = mysqli_prepare($con, $sql_date_check);
