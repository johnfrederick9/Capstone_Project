...
try {
    ...
    $rao_fe_id = isset($_POST['rao_fe_id']) ? intval($_POST['rao_fe_id']) : 4;

    // Validate rao_id
    if ($rao_fe_id <= 0) {
        echo json_encode(['status' => 'false', 'error' => 'Invalid rao_fe_id']);
        exit();
    }

    // Prepare and execute SQL query
    $sql_fetch_attr_name = "SELECT rao_fe_att_id, attribute_name FROM tb_rao_fe_attributes WHERE rao_fe_id = ? AND isDisplayed = 1";
    $stmt_fetch_attr_name = mysqli_prepare($con, $sql_fetch_attr_name);
    mysqli_stmt_bind_param($stmt_fetch_attr_name, "i", $rao_fe_id);
    ...
    $attribute_names = [];
    $attribute_ids = [];  // To store the rao_fe_att_id
    
    // Fetch all rows and collect attribute names and ids
    while ($row = mysqli_fetch_assoc($result)) {
        $attribute_names[] = $row['attribute_name'];
        $attribute_ids[] = $row['rao_fe_att_id'];  // Store the rao_fe_att_id
    }
    
    // Close the statement
    mysqli_stmt_close($stmt_fetch_attr_name);

    $sql_fetch_rao_fe = "SELECT chairman, period_covered, brgy_captain FROM tb_rao_fe WHERE rao_fe_id = ? AND isDisplayed = 1 ";
    $stmt_fetch_rao_fe = mysqli_prepare($con, $sql_fetch_rao_fe);
    mysqli_stmt_bind_param($stmt_fetch_rao_fe, "i", $rao_fe_id);

    if (!mysqli_stmt_execute($stmt_fetch_rao_fe)) {
        throw new Exception("Error executing fetch rao_fe query: " . mysqli_error($con));
    }

    // Fetch the result
    $result_fetch_rao_fe = mysqli_stmt_get_result($stmt_fetch_rao_fe);

    if ($row = mysqli_fetch_assoc($result_fetch_rao_fe)) {
        $chairman = $row['chairman'];
        $period_covered = $row['period_covered'];
        $brgy_captain = $row['brgy_captain'];
    } else {
        throw new Exception("No data found for rao_fe_id: " . $rao_fe_id . " in tb_rao_fe");
    }

    // Close the statement to free resources
    mysqli_stmt_close($stmt_fetch_rao_fe);

    /////////////////////////////// AP

    $rao_fe_ap = []; // Initialize the array for storing fetched rao_fe_ap data
    $rao_fe_ap_data = []; // Initialize the array for storing fetched rao_fe_ap_data

    // Fetch data from tb_rao_fe_ap
    $sql_fetch_rao_fe_ap = "SELECT * FROM tb_rao_fe_ap WHERE rao_fe_id = ?";
    $stmt_fetch_rao_fe_ap = mysqli_prepare($con, $sql_fetch_rao_fe_ap);
    mysqli_stmt_bind_param($stmt_fetch_rao_fe_ap, "i", $rao_fe_id);
    if (!mysqli_stmt_execute($stmt_fetch_rao_fe_ap)) {
        throw new Exception("Error executing fetch rao_fe_ap query: " . mysqli_error($con));
    }
    $rao_fe_ap_result = mysqli_stmt_get_result($stmt_fetch_rao_fe_ap);

    // Fetch rows into $rao_fe_ap
    while ($row = $rao_fe_ap_result->fetch_assoc()) {
        $rao_fe_ap[] = $row; // Append each row to the array
    }

    mysqli_stmt_close($stmt_fetch_rao_fe_ap);

    // Fetch data from tb_rao_fe_ap_data for each rao_fe_ap_id
    foreach ($rao_fe_ap as $apItem) {
        $rao_fe_ap_id = $apItem['rao_fe_ap_id']; // Get rao_fe_ap_id from the row

        $sql_fetch_rao_fe_ap_data = "SELECT * FROM tb_rao_fe_ap_data WHERE rao_fe_ap_id = ? AND isDisplayed = 1 ORDER BY rao_fe_att_id ASC";
        $stmt_fetch_rao_fe_ap_data = mysqli_prepare($con, $sql_fetch_rao_fe_ap_data);
        mysqli_stmt_bind_param($stmt_fetch_rao_fe_ap_data, "i", $rao_fe_ap_id);
        if (!mysqli_stmt_execute($stmt_fetch_rao_fe_ap_data)) {
            throw new Exception("Error executing fetch rao_fe_ap_data query: " . mysqli_error($con));
        }
        $rao_fe_ap_result_data = mysqli_stmt_get_result($stmt_fetch_rao_fe_ap_data);

        // Fetch rows into $rao_fe_ap_data
        while ($row = $rao_fe_ap_result_data->fetch_assoc()) {
            $rao_fe_ap_data[] = $row; // Append each row to the array
        }

        mysqli_stmt_close($stmt_fetch_rao_fe_ap_data);
    }

    //Fetch from tb_rao_fe_ap_totals
    $rao_fe_ap_BF_totals = [];
    $rao_fe_ap_TA_totals = [];
    $rao_fe_ap_grand_totals = [];

    // Fetch data from tb_rao_fe_ap_totals
    $sql_fetch_rao_fe_ap_totals = "SELECT * FROM tb_rao_fe_ap_totals WHERE rao_fe_id = ? AND isDisplayed = 1 ORDER BY rao_fe_att_id ASC";
    $stmt_fetch_rao_fe_ap_totals = mysqli_prepare($con, $sql_fetch_rao_fe_ap_totals);
    mysqli_stmt_bind_param($stmt_fetch_rao_fe_ap_totals, "i", $rao_fe_id);

    if (!mysqli_stmt_execute($stmt_fetch_rao_fe_ap_totals)) {
        throw new Exception("Error executing fetch rao_fe_ap query: " . mysqli_error($con));
    }

    $rao_fe_ap_totals_result = mysqli_stmt_get_result($stmt_fetch_rao_fe_ap_totals);

    // Fetch rows and separate by total_type
    while ($row = $rao_fe_ap_totals_result->fetch_assoc()) {
        if ($row['total_type'] == 'BF') {
            $rao_fe_ap_BF_totals[] = $row;
        } elseif ($row['total_type'] == 'TA') {
            $rao_fe_ap_TA_totals[] = $row;
        }else {
            $rao_fe_ap_grand_totals[] = $row;
        }
    }

    // Close the statement
    mysqli_stmt_close($stmt_fetch_rao_fe_ap_totals);


    //////////////////////////////////// OB

    $rao_fe_ob = []; // Initialize the array for storing fetched rao_fe_ob data
    $rao_fe_ob_data = []; // Initialize the array for storing fetched rao_fe_ob_data

    // Fetch data from tb_rao_fe_ob
    $sql_fetch_rao_fe_ob = "SELECT * FROM tb_rao_fe_ob WHERE rao_fe_id = ?";
    $stmt_fetch_rao_fe_ob = mysqli_prepare($con, $sql_fetch_rao_fe_ob);
    mysqli_stmt_bind_param($stmt_fetch_rao_fe_ob, "i", $rao_fe_id);
    if (!mysqli_stmt_execute($stmt_fetch_rao_fe_ob)) {
        throw new Exception("Error executing fetch rao_fe_ob query: " . mysqli_error($con));
    }
    $rao_fe_ob_result = mysqli_stmt_get_result($stmt_fetch_rao_fe_ob);

    // Fetch rows into $rao_fe_ob
    while ($row = $rao_fe_ob_result->fetch_assoc()) {
        $rao_fe_ob[] = $row; // Append each row to the array
    }

    mysqli_stmt_close($stmt_fetch_rao_fe_ob);

    // Fetch data from tb_rao_fe_ob_data for each rao_fe_ob_id
    foreach ($rao_fe_ob as $obItem) {
        $rao_fe_ob_id = $obItem['rao_fe_ob_id']; // Get rao_fe_ob_id from the row

        $sql_fetch_rao_fe_ob_data = "SELECT * FROM tb_rao_fe_ob_data WHERE rao_fe_ob_id = ? AND isDisplayed = 1 ORDER BY rao_fe_att_id ASC";
        $stmt_fetch_rao_fe_ob_data = mysqli_prepare($con, $sql_fetch_rao_fe_ob_data);
        mysqli_stmt_bind_param($stmt_fetch_rao_fe_ob_data, "i", $rao_fe_ob_id);
        if (!mysqli_stmt_execute($stmt_fetch_rao_fe_ob_data)) {
            throw new Exception("Error executing fetch rao_fe_ob_data query: " . mysqli_error($con));
        }
        $rao_fe_ob_result_data = mysqli_stmt_get_result($stmt_fetch_rao_fe_ob_data);

        // Fetch rows into $rao_fe_ob_data
        while ($row = $rao_fe_ob_result_data->fetch_assoc()) {
            $rao_fe_ob_data[] = $row; // Append each row to the array
        }

        mysqli_stmt_close($stmt_fetch_rao_fe_ob_data);
    }

    //Fetch from tb_rao_fe_ob_totals
    $rao_fe_ob_TO_totals = [];
    $rao_fe_ob_OB_totals = [];
    $rao_fe_ob_AB_totals = [];
    $rao_fe_ob_grand_totals = [];

    // Fetch data from tb_rao_fe_ob_totals
    $sql_fetch_rao_fe_ob_totals = "SELECT * FROM tb_rao_fe_ob_totals WHERE rao_fe_id = ? AND isDisplayed = 1 ORDER BY rao_fe_att_id ASC";
    $stmt_fetch_rao_fe_ob_totals = mysqli_prepare($con, $sql_fetch_rao_fe_ob_totals);
    mysqli_stmt_bind_param($stmt_fetch_rao_fe_ob_totals, "i", $rao_fe_id);

    if (!mysqli_stmt_execute($stmt_fetch_rao_fe_ob_totals)) {
        throw new Exception("Error executing fetch rao_fe_ob query: " . mysqli_error($con));
    }

    $rao_fe_ob_totals_result = mysqli_stmt_get_result($stmt_fetch_rao_fe_ob_totals);

    // Fetch rows and separate by total_type
    while ($row = $rao_fe_ob_totals_result->fetch_assoc()) {
        if ($row['total_type'] == 'TO') {
            $rao_fe_ob_TO_totals[] = $row;
        } elseif ($row['total_type'] == 'OB') {
            $rao_fe_ob_OB_totals[] = $row;
        }elseif($row['total_type'] == 'AB'){
            $rao_fe_ob_AB_totals[] = $row;
        }else{
            $rao_fe_ob_grand_totals[] = $row;
        }
    }

    // Close the statement
    mysqli_stmt_close($stmt_fetch_rao_fe_ob_totals);
    ...
    $response = [
        ...
        'rao_fe_ap' => $rao_fe_ap,
        'rao_fe_ap_data' => $rao_fe_ap_data,
        'rao_fe_ap_BF_totals' =>  $rao_fe_ap_BF_totals,
        'rao_fe_ap_TA_totals' =>  $rao_fe_ap_TA_totals,
        'rao_fe_ob' => $rao_fe_ob,
        'rao_fe_ob_data' => $rao_fe_ob_data,
        'rao_fe_ob_TO_totals' => $rao_fe_ob_TO_totals,
        'rao_fe_ob_OB_totals'  =>$rao_fe_ob_OB_totals,
        'rao_fe_ob_AB_totals' => $rao_fe_ob_AB_totals,
        'rao_fe_ap_grand_totals'=>$rao_fe_ap_grand_totals,
        'rao_fe_ob_grand_totals'=>$rao_fe_ob_grand_totals
...
<div class="rao-container">
            ...
            <div class="rao-header">
                <h1>Report of Appropriations and Obligations(RAO-FE)</h1>
                ...
                <div class="details">
                    ...
                    <div class="info">
                        <label>Fund Source:</label> <input type="text" value="Financial Expenses"  disabled />
                    </div>
                </div>
            </div>
            ...
            <table id = "viewDataTable" class="rao-table">
                ...
                <thead class="appropriations-head">
                    <tr>
                        ...
                        <th colspan="5" class="dynamic-stick-head">Financial Expenses</th><!-- Dynamic Heads max 5 -->
                ...
                <thead class="obligations-head">
                    <tr>
                        ...
                        <th rowspan="2" class="stick-head">Particulars</th> 
                        <th rowspan="2" class="stick-head">Totals</th>
                        <th colspan="5" class="dynamic-stick-head">Financial Expenses</th>
     ...
     function formatPeriodCovered(dateString) {
        ...
        console.log(json);
                ...
                if (json.status === 'true') {
                    ...
                    console.log("totals:",json.rao_fe_ap_grand_totals)
                    console.log("totals:",json.rao_fe_ob_grand_totals)
                    ...
                    // Process dynamic data in rao_fe_ap
if (json.rao_fe_ap && Array.isArray(json.rao_fe_ap) && json.rao_fe_ap.length > 0) {
    json.rao_fe_ap.forEach(function (apItem) {
        let apRow = `
        <tr class="ap-data-row">
            ...
            <td class="hidden">${apItem.rao_fe_ap_id || ''}</td>  <!-- Directly display rao_fe_ap_id -->
        `;

        // Filter the associated ap_fe_ap_data based on rao_fe_ap_id
        if (json.rao_fe_ap_data && Array.isArray(json.rao_fe_ap_data)) {
            const relatedApData = json.rao_fe_ap_data.filter(function (apData) {
                return apData.rao_fe_ap_id === apItem.rao_fe_ap_id;
            });

            if (relatedApData.length > 0) {
                relatedApData.forEach(function (apData) {
                    const attrId = apData.rao_fe_att_id;  // Get the attribute ID
...
if (json.rao_fe_ob && Array.isArray(json.rao_fe_ob) && json.rao_fe_ob.length > 0) {
    json.rao_fe_ob.forEach(function (obItem) {
        let obRow = `
        <tr class="ob-data-row">
            ...
            <td class="hidden">${obItem.rao_fe_ob_id || ''}</td>  <!-- Directly display rao_fe_ob_id -->
        `;

        // Filter the associated ob_fe_ob_data based on rao_fe_ob_id
        if (json.rao_fe_ob_data && Array.isArray(json.rao_fe_ob_data)) {
            const relatedObData = json.rao_fe_ob_data.filter(function (obData) {
                return obData.rao_fe_ob_id === obItem.rao_fe_ob_id;
            });

            if (relatedObData.length > 0) {
                relatedObData.forEach(function (obData) {
                    const attrId = obData.rao_fe_att_id;  // Get the attribute ID
    ...
    // If no `rao_fe_ob` data exists, create an empty row
    let emptyRow = `
    <tr class="ob-data-row">
        <td><span style="visibility: hidden;">Invisible text</span> </td>  <!-- Empty date column -->
        <td> </td>  <!-- Empty reference number column -->
...
rowConfigs.ap.forEach(config => {
    ...
    if (config.includeTotal) {
        ...
        const totalArray = json[`rao_fe_ap_grand_totals`]; // Adjust based on your JSON structure
    ...
    attributeList.forEach((attr, index) => {
        let attrId = json.rao_fe_ap_data.find(apData => apData.attribute_name === attr)?.rao_fe_att_id;
        ...
        if (attrId) {
            ...
            const totalsArray = json[`rao_fe_ap_${config.identifier}_totals`];
            const totalEntry = totalsArray?.find(
                apTotal => apTotal.rao_fe_att_id == attrId
            );
...
rowConfigs.ob.forEach(config => {
    ...
    if (config.includeTotal) {
        ...
        // Retrieve the corresponding total entry based on total type and identifier
        const totalArray = json[`rao_fe_ob_grand_totals`]; // Adjust based on your JSON structure
    ...
    // Add attribute data cells (if any)
    attributeList.forEach((attr, index) => {
        let attrId = json.rao_fe_ob_data.find(obData => obData.attribute_name === attr)?.rao_fe_att_id;
        ...
        if (attrId) {
            ...
            // Access the appropriate totals array directly from the response object
            const totalsArray = json[`rao_fe_ob_${config.identifier}_totals`];
            const totalEntry = totalsArray?.find(
                obTotal => obTotal.rao_fe_att_id == attrId
            );
