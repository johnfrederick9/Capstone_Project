...
try {
    ...
    $update_rao_co_query = "UPDATE tb_rao_co 
                              SET chairman = ?, brgy_captain = ?, period_covered = ? 
                              WHERE rao_co_id = ?";
    $stmt = mysqli_prepare($con, $update_rao_co_query);
    mysqli_stmt_bind_param($stmt, 'sssi', $chairman, $brgy_captain, $period_covered, $rao_co_id);
    mysqli_stmt_execute($stmt);

    // Process AP data
if (isset($_POST['ap_data']) && is_array($_POST['ap_data'])) {
    foreach ($_POST['ap_data'] as $apRow) {
        $rao_co_ap_id = !empty($apRow['rao_co_ap_id']) ? intval($apRow['rao_co_ap_id']) : 0;
        $ap_date = $apRow['date'];
        ...
        if ($rao_co_ap_id > 0) {
            ...
            $query = "UPDATE tb_rao_co_ap 
                      SET ap_ref_date = ?, ap_ref_no = ?, ap_particulars = ?, ap_totals = ? 
                      WHERE rao_co_ap_id = ?";
            $stmt = mysqli_prepare($con, $query);
            mysqli_stmt_bind_param($stmt, 'sssdi', $ap_date, $ap_ref_no, $ap_particulars, $ap_total, $rao_co_ap_id);
        } else {
            // Insert new AP record
            $query = "INSERT INTO tb_rao_co_ap 
                      (rao_co_id, ap_ref_date, ap_ref_no, ap_particulars, ap_totals) 
                      VALUES (?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($con, $query);
            mysqli_stmt_bind_param($stmt, 'isssd', $rao_co_id, $ap_date, $ap_ref_no, $ap_particulars, $ap_total);
        }
        mysqli_stmt_execute($stmt);

        if (!$rao_co_ap_id) {
            $rao_co_ap_id = mysqli_insert_id($con);
        }
        ...
        if (isset($apRow['attributes']) && is_array($apRow['attributes'])) {
            foreach ($apRow['attributes'] as $attributeId => $attributeValue) {
                ...
                $select_query = "SELECT rao_co_ap_data_id FROM tb_rao_co_ap_data 
                                 WHERE rao_co_ap_id = ? AND rao_co_att_id = ?";
                $select_stmt = mysqli_prepare($con, $select_query);
                mysqli_stmt_bind_param($select_stmt, 'ii', $rao_co_ap_id, $attributeId);
                mysqli_stmt_execute($select_stmt);
                mysqli_stmt_store_result($select_stmt);
                
                if (mysqli_stmt_num_rows($select_stmt) > 0) {
                    // If the attribute exists, update it
                    $update_query = "UPDATE tb_rao_co_ap_data 
                                     SET attribute_value = ? 
                                     WHERE rao_co_ap_id = ? AND rao_co_att_id = ?";
                    $update_stmt = mysqli_prepare($con, $update_query);
                    mysqli_stmt_bind_param($update_stmt, 'sii', $attributeValue, $rao_co_ap_id, $attributeId);
                    mysqli_stmt_execute($update_stmt);
                } else {
                    // If the attribute does not exist, insert it
                    $insert_query = "INSERT INTO tb_rao_co_ap_data 
                                     (rao_co_ap_id, rao_co_att_id, attribute_name, attribute_value, isDisplayed) 
                                     VALUES (?, ?, (SELECT attribute_name FROM tb_rao_co_attributes 
                                     WHERE rao_co_att_id = ?), ?, 1)";
                    $insert_stmt = mysqli_prepare($con, $insert_query);
                    mysqli_stmt_bind_param($insert_stmt, 'iiis', $rao_co_ap_id, $attributeId, $attributeId, $attributeValue);
...
if (isset($_POST['ob_data']) && is_array($_POST['ob_data'])) {
    foreach ($_POST['ob_data'] as $obRow) {
        $rao_co_ob_id = !empty($obRow['rao_co_ob_id']) ? intval($obRow['rao_co_ob_id']) : 0;
        $ob_date = $obRow['date'];
        ...
        if ($rao_co_ob_id > 0) {
            ...
            $query = "UPDATE tb_rao_co_ob 
                      SET ob_ref_date = ?, ob_ref_no = ?, ob_particulars = ?, ob_totals = ? 
                      WHERE rao_co_ob_id = ?";
            $stmt = mysqli_prepare($con, $query);
            mysqli_stmt_bind_param($stmt, 'sssdi', $ob_date, $ob_ref_no, $ob_particulars, $ob_total, $rao_co_ob_id);
        } else {
            // Insert new OB record
            $query = "INSERT INTO tb_rao_co_ob 
                      (rao_co_id, ob_ref_date, ob_ref_no, ob_particulars, ob_totals) 
                      VALUES (?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($con, $query);
            mysqli_stmt_bind_param($stmt, 'isssd', $rao_co_id, $ob_date, $ob_ref_no, $ob_particulars, $ob_total);
        }
        mysqli_stmt_execute($stmt);

        if (!$rao_co_ob_id) {
            $rao_co_ob_id = mysqli_insert_id($con);
        }
        ...
        if (isset($obRow['attributes']) && is_array($obRow['attributes'])) {
            foreach ($obRow['attributes'] as $attributeId => $attributeValue) {
                ...
                $select_query = "SELECT rao_co_ob_data_id FROM tb_rao_co_ob_data 
                                 WHERE rao_co_ob_id = ? AND rao_co_att_id = ?";
                $select_stmt = mysqli_prepare($con, $select_query);
                mysqli_stmt_bind_param($select_stmt, 'ii', $rao_co_ob_id, $attributeId);
                mysqli_stmt_execute($select_stmt);
                mysqli_stmt_store_result($select_stmt);
                
                if (mysqli_stmt_num_rows($select_stmt) > 0) {
                    // If the attribute exists, update it
                    $update_query = "UPDATE tb_rao_co_ob_data 
                                     SET attribute_value = ? 
                                     WHERE rao_co_ob_id = ? AND rao_co_att_id = ?";
                    $update_stmt = mysqli_prepare($con, $update_query);
                    mysqli_stmt_bind_param($update_stmt, 'sii', $attributeValue, $rao_co_ob_id, $attributeId);
                    mysqli_stmt_execute($update_stmt);
                } else {
                    // If the attribute does not exist, insert it
                    $insert_query = "INSERT INTO tb_rao_co_ob_data 
                                     (rao_co_ob_id, rao_co_att_id, attribute_name, attribute_value, isDisplayed) 
                                     VALUES (?, ?, (SELECT attribute_name FROM tb_rao_co_attributes 
                                     WHERE rao_co_att_id = ?), ?, 1)";
                    $insert_stmt = mysqli_prepare($con, $insert_query);
                    mysqli_stmt_bind_param($insert_stmt, 'iiis', $rao_co_ob_id, $attributeId, $attributeId, $attributeValue);
                    mysqli_stmt_execute($insert_stmt);
                }
            }
        }
            ...
            foreach ($_POST['ap_totals'] as $identifier => $values) {
                if (is_array($values)) {
                    foreach ($values as $attributeId => $totalValue) {
                        ...
                        $sql_fetch_ap_totals = "SELECT * FROM tb_rao_co_ap_totals WHERE rao_co_att_id = ? AND total_type = ? AND rao_co_id = ? AND isDisplayed = 1";
                        $stmt_fetch_ap_totals = mysqli_prepare($con, $sql_fetch_ap_totals);
                        mysqli_stmt_bind_param($stmt_fetch_ap_totals, "isi", $attributeId, $identifier, $rao_co_id);
                        ...
                        if (mysqli_stmt_num_rows($stmt_fetch_ap_totals) > 0) {
                            ...
                            $sql_update_ap_total = "UPDATE tb_rao_co_ap_totals 
                                                    SET attribute_value = ? 
                                                    WHERE rao_co_att_id = ? AND total_type = ? AND rao_co_id = ? AND isDisplayed = 1";
                            $stmt_update_ap_total = mysqli_prepare($con, $sql_update_ap_total);
                            mysqli_stmt_bind_param($stmt_update_ap_total, 'disi', $totalValue, $attributeId, $identifier, $rao_co_id);
                            ...
                            $sql_fetch_att_name = "SELECT attribute_name FROM tb_rao_co_attributes WHERE rao_co_att_id = ? AND isDisplayed = 1";
                            ...
                            // Insert new AP total
                            $sql_insert_ap_total = "INSERT INTO tb_rao_co_ap_totals 
                            ...
                            mysqli_stmt_bind_param($stmt_insert_ap_total, 'isisd', $rao_co_id, $identifier, $attributeId, $attributeName, $totalValue);
                    ...
                    $sql_fetch_ap_totals = "SELECT * FROM tb_rao_co_ap_totals WHERE total_type = ? AND rao_co_id = ? AND isDisplayed = 1";
                    $stmt_fetch_ap_totals = mysqli_prepare($con, $sql_fetch_ap_totals);
                    mysqli_stmt_bind_param($stmt_fetch_ap_totals, "si", $identifier, $rao_co_id);
                    ...
                    if (mysqli_stmt_num_rows($stmt_fetch_ap_totals) > 0) {
                        ...
                        $sql_update_ap_total = "UPDATE tb_rao_co_ap_totals 
                                                SET attribute_value = ? 
                                                WHERE total_type = ? AND rao_co_id = ? AND isDisplayed = 1";
                        $stmt_update_ap_total = mysqli_prepare($con, $sql_update_ap_total);
                        mysqli_stmt_bind_param($stmt_update_ap_total, 'dsi', $values, $identifier, $rao_co_id);
                        ...
                        $sql_insert_ap_total = "INSERT INTO tb_rao_co_ap_totals (rao_co_id, total_type, attribute_value, isDisplayed) 
                                                VALUES (?, ?, ?, 1)";
                        $stmt_insert_ap_total = mysqli_prepare($con, $sql_insert_ap_total);
                        mysqli_stmt_bind_param($stmt_insert_ap_total, 'isd', $rao_co_id, $identifier, $values);
        ...
        foreach ($_POST['ob_totals'] as $identifier => $values) {
            if (is_array($values)) {
                foreach ($values as $attributeId => $totalValue) {
                    ...
                    $sql_fetch_ob_totals = "SELECT * FROM tb_rao_co_ob_totals 
                                            WHERE rao_co_att_id = ? AND total_type = ? AND rao_co_id = ? AND isDisplayed = 1";
                    $stmt_fetch_ob_totals = mysqli_prepare($con, $sql_fetch_ob_totals);
                    mysqli_stmt_bind_param($stmt_fetch_ob_totals, "isi", $attributeId, $identifier, $rao_co_id);
                    ...
                    if (mysqli_stmt_num_rows($stmt_fetch_ob_totals) > 0) {
                        // Record exists, update the total value
                        $sql_update_ob_total = "UPDATE tb_rao_co_ob_totals 
                                                SET attribute_value = ? 
                                                WHERE rao_co_att_id = ? AND total_type = ? AND rao_co_id = ? AND isDisplayed = 1";
                        $stmt_update_ob_total = mysqli_prepare($con, $sql_update_ob_total);
                        mysqli_stmt_bind_param($stmt_update_ob_total, 'disi', $totalValue, $attributeId, $identifier, $rao_co_id);
                        ...
                        // Insert new OB total
                        $sql_insert_ob_total = "INSERT INTO tb_rao_co_ob_totals 
                                              (rao_co_id, total_type, rao_co_att_id, attribute_name, attribute_value, isDisplayed) 
                                              VALUES (?, ?, ?, ?, ?, 1)";
                        $stmt_insert_ob_total = mysqli_prepare($con, $sql_insert_ob_total);
                        mysqli_stmt_bind_param($stmt_insert_ob_total, 'isisd', $rao_co_id, $identifier, $attributeId, $attributeName, $totalValue);
                ...
                // Check if the total already exists
                $sql_fetch_ob_totals = "SELECT * FROM tb_rao_co_ob_totals WHERE total_type = ? AND rao_co_id = ? AND isDisplayed = 1";
                $stmt_fetch_ob_totals = mysqli_prepare($con, $sql_fetch_ob_totals);
                mysqli_stmt_bind_param($stmt_fetch_ob_totals, "si", $identifier, $rao_co_id);
                ...
                if (mysqli_stmt_num_rows($stmt_fetch_ob_totals) > 0) {
                    ...
                    $sql_update_ob_total = "UPDATE tb_rao_co_ob_totals 
                                            ...
                                            WHERE total_type = ? AND rao_co_id = ? AND isDisplayed = 1";
                    $stmt_update_ob_total = mysqli_prepare($con, $sql_update_ob_total);
                    mysqli_stmt_bind_param($stmt_update_ob_total, 'dsi', $values, $identifier, $rao_co_id);
                    ...
                    // Total doesn't exist, insert a new record
                    $sql_insert_ob_total = "INSERT INTO tb_rao_co_ob_totals (rao_co_id, total_type, attribute_value, isDisplayed) 
                                            VALUES (?, ?, ?, 1)";
                    $stmt_insert_ob_total = mysqli_prepare($con, $sql_insert_ob_total);
                    mysqli_stmt_bind_param($stmt_insert_ob_total, 'isd', $rao_co_id, $identifier, $values);
