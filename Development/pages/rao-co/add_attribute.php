...
try {
    // Insert a temporary blank data row into tb_rao_co to get the id
    $sql_insert_blank = "INSERT INTO tb_rao_co (chairman, period_covered, brgy_captain, isDisplayed)
    ...
    if (!$stmt_insert_blank || !mysqli_stmt_execute($stmt_insert_blank)) {
        throw new Exception("Failed to insert blank data into tb_rao_co: " . mysqli_error($con));
    }

    // Get the newly inserted rao_id
    $rao_co_id = mysqli_insert_id($con);
    ...
    foreach ($attribute_name as $index => $name) {
        $sql_insert_attributes = "INSERT INTO tb_rao_co_attributes (rao_co_id, attribute_name, isDisplayed) VALUES (?, ?, 1)";
        $stmt_insert_attributes = mysqli_prepare($con, $sql_insert_attributes);
        
        if (!$stmt_insert_attributes) {
            throw new Exception("Failed to prepare statement for tb_rao_co_attributes: " . mysqli_error($con));
        }

        mysqli_stmt_bind_param($stmt_insert_attributes, "is", $rao_co_id, $name);
        
        if (!mysqli_stmt_execute($stmt_insert_attributes)) {
            throw new Exception("Failed to insert into tb_rao_co_attributes: " . mysqli_stmt_error($stmt_insert_attributes));
        }
    ...
    $response = array(
        'status' => 'true',
        'rao_co_id' => $rao_co_id, // Total records before filtering
