...
if ($stmt_fetch_previous) {
    ...
    $sql_monthly_up = "UPDATE tb_cashbook_monthly SET date_data = ?,
            clt_init_balance = ?, clt_end_balance = ?, isDefaultClt = ?,
            cb_init_balance = ?, cb_end_balance = ?, isDefaultCb = ?
        WHERE cashbook_id = ?";
    $stmt_monthly_up = mysqli_prepare($con, $sql_monthly_up);
    mysqli_stmt_bind_param($stmt_monthly_up, "sddiddii", 
     $period_covered,$clt_init_balance,$clt_end_balance, $isDefaultClt,
        $cb_init_balance, $cb_end_balance, $isDefaultCb, 
        $cashbook_id
    );
