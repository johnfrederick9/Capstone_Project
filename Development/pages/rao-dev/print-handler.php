...
function formatValue($value) {
    ...
    $rao_dev_id = isset($_POST['rao_dev_id']) ? intval($_POST['rao_dev_id']) : '7';
...
<body>
    <div class="rao-container">
        <div class="rao-header">
            <h1>Report of Appropriations and Obligations(RAO-DEV-FUND)</h1>
...
if (!empty($rao_dev_ap)) {
    ...
    $apDataLookup = [];
    foreach ($rao_dev_ap_data as $apData) {
        $key = $apData['rao_dev_ap_id'] . '_' . $apData['rao_dev_att_id'];
    ...
    foreach ($rao_dev_ap as $apItem) {
        echo '<tr class="ap-data-row">
            ...
            <td class="hidden">' . htmlspecialchars($apItem['rao_dev_ap_id'] ?? '') . '</td>';

        // Output attribute values using lookup array
        foreach ($attribute_ids as $attrId) {
            $key = $apItem['rao_dev_ap_id'] . '_' . $attrId;
...
echo '<tbody class="inp-group-ap-totals TA">
    <tr class="totals-row">
        ...
        <td class="total-data">' . htmlspecialchars(formatValue($rao_dev_ap_grand_totals[0]['attribute_value'] ?? '')) . '</td>';

// Add AP total values for each attribute column
foreach ($attribute_ids as $attrId) {
    $value = '';
    foreach ($rao_dev_ap_TA_totals as $total) {
        if ($total['rao_dev_att_id'] == $attrId) {
...
echo '<tbody class="inp-group-ap-totals BF">
    <tr class="totals-row">
        ...
        <td class="total-data">' . htmlspecialchars(formatValue($rao_dev_ap_grand_totals[1]['attribute_value'] ?? '')) . '</td>';

foreach ($attribute_ids as $attrId) {
    $value = '';
    foreach ($rao_dev_ap_BF_totals as $total) {
        if ($total['rao_dev_att_id'] == $attrId) {
            $value = $total['attribute_value'];
            break;
        }
...
if (!empty($rao_dev_ob)) {
    foreach ($rao_dev_ob as $obItem) {
        echo '<tr class="ob-data-row">
            ...
            <td class="hidden">' . htmlspecialchars($obItem['rao_dev_ob_id'] ?? '') . '</td>';

        foreach ($attribute_ids as $attrId) {
            $value = '';
            foreach ($rao_dev_ob_data as $obData) {
                if ($obData['rao_dev_ob_id'] === $obItem['rao_dev_ob_id'] && 
                    $obData['rao_dev_att_id'] === $attrId) {
...
$totalTypes = [
    [
        ...
        'data' => $rao_dev_ob_TO_totals,
        'grand_total' => $rao_dev_ob_grand_totals[0]['attribute_value'] ?? ''
    ],
    [
        'class' => 'OB', 
        'label' => 'Obligations Balance To Date', 
        'data' => $rao_dev_ob_OB_totals,
        'grand_total' => $rao_dev_ob_grand_totals[1]['attribute_value'] ?? ''
    ],
    [
        'class' => 'AB', 
        'label' => 'Appropriations Balance To Date', 
        'data' => $rao_dev_ob_AB_totals,
        'grand_total' => $rao_dev_ob_grand_totals[2]['attribute_value'] ?? ''
    ]
];
