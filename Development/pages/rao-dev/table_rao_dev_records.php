    ...
    <section class="home">  
        <div class="financial_rao">
                <div class="table-container">
                    ...
                    <div class="head">
                            <h1>Records of Appropriations and Obligations (RAO-DEV)</h1>
                        </div>
                    ...
                    <script type="text/javascript">
                        ...
                        $('#example').on('click', '.editbtn', function (event) {
                            ...
                            var rao_dev_id = $(this).data('id');
                            console.log(trid);

                            // Store data in modal for reference
                            $('#updateUser').data('rao_dev_id', rao_dev_id);
                            $('#updateUser').data('trid', trid);

                            // Show modal and set rao_dev_id value
                            $('#exampleModal').modal('show');
                            console.log("Rao dev ID:", rao_dev_id);
                            $('#exampleModal #rao_dev_id').val(rao_dev_id);
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    try {
                                            ...
                                            if (json[dataKey] && Array.isArray(json[dataKey]) && json[dataKey].length > 0) {
                                                json[dataKey].forEach(function (item) {
                                                    ...
                                                    <tr class="${type}-data-row">
                                                        ...
                                                        <td class="hidden"><input type="hidden" name="rao_dev_${type}_id" value="${item[`rao_dev_${type}_id`] || ''}"></td>
                                                    `;
                                                    ...
                                                    if (json[`${dataKey}_data`] && Array.isArray(json[`${dataKey}_data`])) {
                                                        const relatedData = json[`${dataKey}_data`].filter(function (data) {
                                                            return data[`rao_dev_${type}_id`] === item[`rao_dev_${type}_id`];
                                                        });

                                                        relatedData.forEach(function (data) {
                                                            const attrId = data[`rao_dev_att_id`];
                                        ...
                                        createDynamicRow('ap', 'rao_dev_ap', 'inp-group-ap-data-row', 'ap');

                                        // Call the function for OB data
                                        createDynamicRow('ob', 'rao_dev_ob', 'inp-group-ob-data-row', 'ob');
                                            ...
                                            rowConfigs.ap.forEach(config => {
                                                ...
                                                attributeList.forEach((attr, index) => {
                                                    let attrId = json.rao_dev_ap_data.find(apData => apData.attribute_name === attr)?.rao_dev_att_id;
                                            ...
                                            rowConfigs.ob.forEach(config => {
                                                ...
                                                attributeList.forEach((attr, index) => {
                                                    let attrId = json.rao_dev_ob_data.find(obData => obData.attribute_name === attr)?.rao_dev_att_id;
                       ...
                       // Function to format the period covered
                        ...
                        $('#example').on('click', '.infoBtn', function (event) {
                            ...
                            var rao_dev_id = $(this).data('item_id');
                            ...
                            console.log("Rao dev ID:", rao_dev_id);
                            $('#viewDataModal #rao_dev_id').val(rao_dev_id);
                                ...
                                success: function (data) {
                                    try {
                                        ...
                                        if (json.status === 'true') {
                                           ...
                                           // Process dynamic data in rao_dev_ap
                                            if (json.rao_dev_ap && Array.isArray(json.rao_dev_ap) && json.rao_dev_ap.length > 0) {
                                                json.rao_dev_ap.forEach(function (apItem) {
                                                    let apRow = `
                                                    <tr class="ap-data-row">
                                                        ...
                                                        <td class="hidden"><input type="hidden" name="rao_dev_ap_id" value="${apItem.rao_dev_ap_id || ''}" disabled></td>
                                                    `;

                                                    // Filter the associated ap_dev_ap_data based on rao_dev_ap_id
                                                    if (json.rao_dev_ap_data && Array.isArray(json.rao_dev_ap_data)) {
                                                        const relatedApData = json.rao_dev_ap_data.filter(function (apData) {
                                                            return apData.rao_dev_ap_id === apItem.rao_dev_ap_id;
                                                        });

                                                        if (relatedApData.length > 0) {
                                                            relatedApData.forEach(function (apData) {
                                                                const attrId = apData.rao_dev_att_id;  // Get the attribute ID
                                                ...
                                                <tr class="ap-data-row">
                                                    ...
                                                    <td class="hidden"><input type="hidden" name="rao_dev_ap_id" value="" disabled></td>
                                                `;
                                             ...
                                             if (json.rao_dev_ob && Array.isArray(json.rao_dev_ob) && json.rao_dev_ob.length > 0) {
                                                json.rao_dev_ob.forEach(function (obItem) {
                                                    let obRow = `
                                                    <tr class="ob-data-row">
                                                        ...
                                                        <td class="hidden"><input type="hidden" name="rao_dev_ob_id" value="${obItem.rao_dev_ob_id || ''}" disabled></td>
                                                    `;

                                                    // Filter the associated ob_dev_ob_data based on rao_dev_ob_id
                                                    if (json.rao_dev_ob_data && Array.isArray(json.rao_dev_ob_data)) {
                                                        const relatedObData = json.rao_dev_ob_data.filter(function (obData) {
                                                            return obData.rao_dev_ob_id === obItem.rao_dev_ob_id;
                                                        });

                                                        if (relatedObData.length > 0) {
                                                            relatedObData.forEach(function (obData) {
                                                                const attrId = obData.rao_dev_att_id;  // Get the attribute ID
                                                ...
                                                // If no `rao_dev_ob` data exists, create an empty row
                                                let emptyRow = `
                                                <tr class="ob-data-row">
                                                    ...
                                                    <td class="hidden"><input type="hidden" name="rao_dev_ob_id" value="" disabled></td>
                                                `;
                        ...
                        var rao_dev_id = $(this).data('rao_id'); // Get ID from data attribute
                        var table = $('#example').DataTable();
                            ...
                            if (attribute_name.length > 0 && !attribute_name.includes("")) {
                                $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        if (status == 'true') {
                                        // Fetch rao_dev_id from the response
                                        var rao_dev_id = json.rao_dev_id;

                                        // Hide the AttributeModal
                                        $('#AttributeModal').modal('hide');
                                        showAlert('Attributes added successfully!',"alert-success");

                                        // Set the rao_dev_id in the addUserModal's input field
                                        $('#addUserModal #rao_dev_id').val(rao_dev_id);
                        ...
                        $(document).on('submit', '#updateAttributeForm', function (e) {
                            ...
                            let rao_dev_id = $('#UpdateAttributeModal #rao_dev_id').val();
                            let visibleModal = null;
                            
                            if (!rao_dev_id) {
                                ['#addUserModal', '#exampleModal'].forEach((modalSelector) => {
                                    if ($(modalSelector).is(':visible')) {
                                        rao_dev_id = $(`${modalSelector} #rao_dev_id`).val();
                            ...
                            console.log("rao_dev_id:", rao_dev_id);

                            if (!rao_dev_id) {
                                showAlert('Error: RAO dev ID is missing.',"alert-danger");
                                return;
                            }
                            ...
                            rows.each(function() {
                                const row = $(this);
                                const rao_dev_att_id = row.find('input[name="rao_dev_att_id[]"]').val();
                            ...
                            console.log("Form Data Structure:", {
                                ...
                                rao_dev_att_id: attributeData.map(item => item.rao_dev_att_id),
                                column_name: attributeData.map(item => item.column_name)
                            });

                            formData.append('rao_dev_id', rao_dev_id);
                            formData.append('rao_dev_att_id', JSON.stringify(attributeData.map(item => item.rao_dev_att_id)));
                                        ...
                                        if (json.restored_attributes && json.restored_attributes.length > 0) {
                                            console.log("restored:", json.restored_attributes);
                                                ...
                                                $('.inp-group-ap-data-row tr.ap-data-row').each(function() {
                                                    ...
                                                    json.restored_attributes.forEach(function(attr) {
                                                        const newCell = `<td>
                                                            <input type="number" name="ap_attr_${attr.rao_dev_att_id}[]" value="" step="0.01">
                                                        </td>`;
                                                ...
                                                $('.inp-group-ob-data-row tr.ob-data-row').each(function() {
                                                    ...
                                                    json.restored_attributes.forEach(function(attr) {
                                                        const newCell = `<td>
                                                            <input type="number" name="ob_attr_${attr.rao_dev_att_id}[]" value="" step="0.01">
                                                        </td>`;
                                                ...
                                                totalRowsConfig.forEach(config => {
                                                    $(config.selector).each(function() {
                                                        ...
                                                        json.restored_attributes.forEach(function(attr) {
                                                            const newCell = `<td>
                                                                <input type="number" name="${config.prefix}_attr_${config.type}_${attr.rao_dev_att_id}" disabled="">
                                                            </td>`;
                                                            actionCell.before(newCell);
                                                        });
                                                    });
                        ...
                        $(document).on('submit', '#addUser', function (e) {
                            ...
                            let formData = {
                                rao_dev_id: $('#addUserModal #rao_dev_id').val(),
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    if (response.status === 'true') {
                                        ...
                                        showAlert('Error saving data: ' + response.error, "alert-success");
                                    }
                                },
                                error: function (jqXHR, textStatus, errorThrown) {
                                    console.error('AJAX Error: ' + textStatus, errorThrown);
                                    console.error('Response: ' + jqXHR.responseText);
                                    showAlert('Error: ' + errorThrown, "alert-success");
                                }
                            });
                        });
                        ...
                        $(document).on('submit', '#updateUser', function(e) {
                            ...
                            let formData = {
                                rao_dev_id: $('#exampleModal #rao_dev_id').val(),
                            ...
                            $('#exampleModal .inp-group-ap-data-row .ap-data-row').each(function () {
                                let apRowData = {
                                    rao_dev_ap_id: $(this).find('input[name="rao_dev_ap_id"]').val(),
                                    date: $(this).find('input[name="ap_date_data[]"]').val(),
                                    reference_no: $(this).find('input[name="ap_reference_no[]"]').val(),
                                    particulars: $(this).find('input[name="ap_particulars[]"]').val(),
                                    total: $(this).find('input[name="ap_total[]"]').val(),
                            ...
                            $('#exampleModal .inp-group-ob-data-row .ob-data-row').each(function () {
                                let obRowData = {
                                    rao_dev_ob_id: $(this).find('input[name="rao_dev_ob_id"]').val(),
                                    date: $(this).find('input[name="ob_date_data[]"]').val(),
                                    reference_no: $(this).find('input[name="ob_reference_no[]"]').val(),
                                    particulars: $(this).find('input[name="ob_particulars[]"]').val(),
                                    total: $(this).find('input[name="ob_total[]"]').val(),
                                ...
                                success: function (response) {
                                    if (response.status === 'true') {
                                        ...
                                        var button = `
                                                <td>
                                                    <div class="buttons">
                                                        <a href="javascript:void(0);" data-id="${rao_dev_id}" class="update-btn btn-sm editbtn">
                                                            <i class="bx bx-edit"></i>
                                                        </a>  
                                                        <a href="!#;" data-rao_id="${rao_dev_id}" class="delete-btn btn-sm deleteBtn">
                                                            <i class="bx bxs-trash"></i>
                                                        </a>
                                                        <a href="!#;" data-item-id="${rao_dev_id}" class="update-btn btn-sm infoBtn">
                        ...
                        var rao_dev_id = $('#viewDataModal #rao_dev_id').val(); // Get the ID from hidden input
                        console.log("Print", rao_dev_id); //
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Add New Column for Report of Appropriations and Obligations (RAO-DEV)</h5>
                            <button type="button" class='bx bxs-x-circle icon' data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        
                        <div class="modal-header">
                            <div class="dropdown" data-control="checkbox-dropdown">
                                <label>List of DEV: </label>
            ...
            <div class="modal fade" id="UpdateAttributeModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <input type="hidden" id="rao_dev_id" name="rao_dev_id">
                                <div class="attribute-container">
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Report of Appropriations and Obligations (RAO-DEV)</h5>
                            <button type="button" class='bx bxs-x-circle icon' data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="rao-container">
                                ...
                                <div class="rao-header">
                                    <h1>Report of Appropriations and Obligations (RAO-DEV)</h1>
            ...
            <!-- Update RAO -->
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Update Report of Appropriations and Obligations (RAO-DEV)</h5>
            ...
            <div class="modal fade" id="addUserModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Add Report of Appropriations and Obligations (RAO-DEV)</h5>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchData() {
        ...
        ['#addUserModal', '#exampleModal', '#UpdateAttributeModal'].forEach((modalId) => {
            if ($(modalId).is(':visible')) {
                raoDevId = $(`${modalId} #rao_dev_id`).val();
            }
        });

        if (!raoDevId) {
            console.error('No active modal or rao_dev_id not found');
            showAlert('Unable to fetch the ID. Please ensure the modal is active.', "alert-danger");
            return;
        }

        console.log("raoDevId: ", raoDevId);
        
        $(`#UpdateAttributeModal #rao_dev_id`).val(raoDevId);
        const columnLists = document.querySelector("#UpdateAttributeModal .column-lists");

        $.ajax({
            url: "fetch_table_columns.php",
            data: { rao_dev_id: raoDevId},
            type: 'GET',
            dataType: 'json',
            success: function (response) {
                ...
                if (response.attributes && response.attributes.length > 0) {
                    response.attributes.forEach((attribute, index) => {
                        addRow({
                            counter: index + 1,
                            rao_dev_att_id: attribute.rao_dev_att_id,
    ...
    newRow.innerHTML = `
        ...
        <td class="hidden">
            <input type="hidden" name="rao_dev_att_id[]" value="${rowData.rao_dev_att_id}">
        </td>
        <td>
    ...
    checkboxes.forEach(function (checkbox) {
        ...
        if (checkbox.checked) {
            ...
            IdInput.setAttribute('name', 'rao_dev_att_id[]');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeModal1() {
        ...
        const raoDevId = document.querySelector('#addUserModal #rao_dev_id');

        if (raoDevId) {
            console.log('rao_dev_id found:', raoDevId.value);
        } else {
            console.error('rao_dev_id not found');
        }
        ...
        periodCoveredInput.addEventListener('change', function() {
            ...
            fetch(`get_monthly.php?task=${task}&date=${selectedDate}&id=${raoDevId.value}`)
    ...
    types.forEach(type => {
        ...
        rowConfigs[type].forEach(config => {
            ...
            attributes.forEach(attr => {
                const attrId = attr.rao_dev_att_id;
                const td = document.createElement('td');
                const input = document.createElement('input');
                input.type = 'number';
    ...
    obTotalsInitialized = true;
        ...
        updateDateInputs();
        //Dynamic Rows
        function addRow(type, afterElement = null, raoDevIdValue) {
        ...
        document.querySelector('#addUserModal').addEventListener('click', function(event) {
            if (event.target.classList.contains('add-row-ap')) {
                ...
                addRow('ap', currentRow, raoDevId.value);
                updateDateInputs();
            } else if (event.target.classList.contains('add-row-ob')) {
                event.preventDefault();
                const currentRow = event.target.closest('tr');
                addRow('ob', currentRow, raoDevId.value);
                updateDateInputs();
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeModal1() {
        ...
        const raoDevId = document.querySelector('#exampleModal #rao_dev_id');
        ...
        periodCoveredInput.addEventListener('change', function() {
            ...
            console.log("Period Covered:", selectedDate);
            console.log("raoDevId get monthly:", raoDevId.value);
            fetch(`get_monthly.php?task=${task}&date=${selectedDate}&id=${raoDevId.value}`)
                .then(response => response.json())
                .then(data => {
                    if (data.status === 'true') {
        ...
        function addRow(type, afterElement = null, raoDevIdValue) { 
            ...
            const baseCells = [
                ...
                createCell('input', '', {type: 'text', name: `${type}_particulars[]`,  required: false}),
                createCell('input', '', {type: 'number', name: `${type}_total[]`,  step: 0.01}, 'total-data'),
                createCell('input', '', {type: 'hidden', name: `rao_dev_${type}_id`}, 'hidden'),
            ];
