...
try {
    ...
    if($task == "update"){
        // $sql_fetch_date = "SELECT period_covered FROM tb_rao_dev WHERE isDisplayed = 1 AND rao_dev_id = ?";
        ...
        // }
         ...
         $sql_date_check = "SELECT COUNT(*) as month_count FROM tb_rao_dev WHERE DATE_FORMAT(period_covered, '%Y-%m') = ? AND isDisplayed = 1 AND rao_dev_id != ?";
        ...
        $sql_date_check = "SELECT COUNT(*) as month_count FROM tb_rao_dev WHERE DATE_FORMAT(period_covered, '%Y-%m') = ? AND isDisplayed = 1";
        $stmt_date_check = mysqli_prepare($con, $sql_date_check);
