...
try {
    ...
    $rao_sk_id = isset($_POST['rao_sk_id']) ? intval($_POST['rao_sk_id']) : '';
    ...
    $response = [
        ...
        'rao_sk_ap_grand_totals'=>$rao_sk_ap_grand_totals,
        'rao_sk_ob_grand_totals'=>$rao_sk_ob_grand_totals
...
<div class="rao-container">
            ...
            <div class="rao-header">
                ...
                <div class="details">
                    ...
                    <div class="info">
                        <label>Fund Source:</label> <input type="text" value="10% Sangguniang Kabataan FUNDS"  disabled />
                    </div>
                </div>
            </div>
            ...
            <table id = "viewDataTable" class="rao-table">
                ...
                <thead class="appropriations-head">
                    <tr>
                        ...
                        <th colspan="5" class="dynamic-stick-head">10% SK FUNDS</th><!-- Dynamic Heads max 5 -->
                ...
                <thead class="obligations-head">
                    <tr>
                        ...
                        <th rowspan="2" class="stick-head">Particulars</th> 
                        <th rowspan="2" class="stick-head">Totals</th>
                        <th colspan="5" class="dynamic-stick-head">10% SK FUNDS</th>
     ...
     function formatPeriodCovered(dateString) {
        ...
        console.log(json);
                ...
                if (json.status === 'true') {
                    ...
                    console.log("totals:",json.rao_sk_ap_grand_totals)
                    console.log("totals:",json.rao_sk_ob_grand_totals)
...
//             <td class="hidden">${apItem.rao_sk_ap_id || ''}</td>  <!-- Directly display rao_sk_ap_id -->
//         `;
...
// } else {
    ...
    json.rao_sk_ap.forEach(function (apItem) {
        ...
        const formattedApTotal = apItem.ap_totals ? parseFloat(apItem.ap_totals).toLocaleString() : '';
        ...
        <tr class="ap-data-row">
            ...
            <td>${apItem.ap_ref_no || ''}</td>  <!-- Directly display reference number -->
            <td>${apItem.ap_particulars || ''}</td>  <!-- Directly display particulars -->
            <td class="total-data">${formattedApTotal}</td>  <!-- Directly display formatted totals -->
            <td class="hidden">${apItem.rao_sk_ap_id || ''}</td>  <!-- Directly display rao_sk_ap_id -->
        `;
        ...
        if (json.rao_sk_ap_data && Array.isArray(json.rao_sk_ap_data)) {
            ...
            if (relatedApData.length > 0) {
                relatedApData.forEach(function (apData) {
                    ...
                    // Format the attribute value with commas, if it's a number
                    const formattedAttributeValue = apData.attribute_value ? parseFloat(apData.attribute_value).toLocaleString() : '';

                    // Create a new <td> with the formatted attribute value for each related apData
                    apRow += `
                    <td>${formattedAttributeValue || ''}</td>  <!-- Directly display formatted attribute value -->
                    `;
                });
            }
        }
...
//             <td class="hidden">${obItem.rao_sk_ob_id || ''}</td>  <!-- Directly display rao_sk_ob_id -->
//         `;
...
if (json.rao_sk_ob && Array.isArray(json.rao_sk_ob) && json.rao_sk_ob.length > 0) {
    json.rao_sk_ob.forEach(function (obItem) {
        // Format the total with commas
        const formattedObTotal = obItem.ob_totals ? parseFloat(obItem.ob_totals).toLocaleString() : '';
        ...
        <tr class="ob-data-row">
            ...
            <td>${obItem.ob_ref_no || ''}</td>  <!-- Directly display the reference number -->
            <td>${obItem.ob_particulars || ''}</td>  <!-- Directly display the particulars -->
            <td class="total-data">${formattedObTotal}</td>  <!-- Directly display formatted totals -->
            <td class="hidden">${obItem.rao_sk_ob_id || ''}</td>  <!-- Directly display rao_sk_ob_id -->
        `;
        ...
        if (json.rao_sk_ob_data && Array.isArray(json.rao_sk_ob_data)) {
            ...
            if (relatedObData.length > 0) {
                relatedObData.forEach(function (obData) {
                    ...
                    // Format the attribute value with commas, if it's a number
                    const formattedAttributeValue = obData.attribute_value ? parseFloat(obData.attribute_value).toLocaleString() : '';

                    // Create a new <td> with the formatted attribute value for each related obData
                    obRow += `
                    <td>${formattedAttributeValue || ''}</td>  <!-- Directly display formatted attribute value -->
                    `;
                });
...
rowConfigs.ap.forEach(config => {
    ...
    if (config.includeTotal) {
        ...
        const totalArray = json[`rao_sk_ap_grand_totals`]; // Adjust based on your JSON structure
        ...
        if (totalEntry && totalEntry.attribute_value !== undefined) {
            ...
            if (attributeValue == 0 || attributeValue == '0') {
                ...
                totalCell.textContent = parseFloat(attributeValue).toLocaleString();  // Otherwise, display the actual value
            }
        } else {
    ...
    attributeList.forEach((attr, index) => {
        ...
        if (attrId) {
            ...
            const totalsArray = json[`rao_sk_ap_${config.identifier}_totals`];
            const totalEntry = totalsArray?.find(
                apTotal => apTotal.rao_sk_att_id == attrId
            );
            ...
            td.textContent = (totalValue == 0) ? " " : parseFloat(totalValue).toLocaleString();
...
rowConfigs.ob.forEach(config => {
    ...
    if (config.includeTotal) {
        ...
        // Retrieve the corresponding total entry based on total type and identifier
        const totalArray = json[`rao_sk_ob_grand_totals`]; // Adjust based on your JSON structure
    ...
    attributeList.forEach((attr, index) => {
        ...
        if (attrId) {
            ...
            // Access the appropriate totals array directly from the response object
            const totalsArray = json[`rao_sk_ob_${config.identifier}_totals`];
            const totalEntry = totalsArray?.find(
                obTotal => obTotal.rao_sk_att_id == attrId
            );
