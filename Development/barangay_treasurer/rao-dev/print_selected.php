...
try {
    ...
    $rao_dev_id = isset($_POST['rao_dev_id']) ? intval($_POST['rao_dev_id']) : '';
    ...
    $response = [
        ...
        'rao_dev_ap_grand_totals'=>$rao_dev_ap_grand_totals,
        'rao_dev_ob_grand_totals'=>$rao_dev_ob_grand_totals
     ...
     function formatPeriodCovered(dateString) {
        ...
        console.log(json);
                ...
                if (json.status === 'true') {
                    ...
                    console.log("totals:",json.rao_dev_ap_grand_totals)
                    console.log("totals:",json.rao_dev_ob_grand_totals)
...
if (json.rao_dev_ap && Array.isArray(json.rao_dev_ap) && json.rao_dev_ap.length > 0) {
    json.rao_dev_ap.forEach(function (apItem) {
        ...
        <tr class="ap-data-row">
            ...
            <td class="hidden">${apItem.rao_dev_ap_id || ''}</td>  <!-- Directly display rao_dev_ap_id -->
        `;
...
if (json.rao_dev_ob && Array.isArray(json.rao_dev_ob) && json.rao_dev_ob.length > 0) {
    json.rao_dev_ob.forEach(function (obItem) {
        ...
        <tr class="ob-data-row">
            ...
            <td class="hidden">${obItem.rao_dev_ob_id || ''}</td>  <!-- Directly display rao_dev_ob_id -->
        `;
...
rowConfigs.ap.forEach(config => {
    ...
    if (config.includeTotal) {
        ...
        const totalArray = json[`rao_dev_ap_grand_totals`]; // Adjust based on your JSON structure
    ...
    attributeList.forEach((attr, index) => {
        ...
        if (attrId) {
            ...
            const totalsArray = json[`rao_dev_ap_${config.identifier}_totals`];
            const totalEntry = totalsArray?.find(
                apTotal => apTotal.rao_dev_att_id == attrId
            );
...
rowConfigs.ob.forEach(config => {
    ...
    if (config.includeTotal) {
        ...
        // Retrieve the corresponding total entry based on total type and identifier
        const totalArray = json[`rao_dev_ob_grand_totals`]; // Adjust based on your JSON structure
    ...
    attributeList.forEach((attr, index) => {
        ...
        if (attrId) {
            ...
            // Access the appropriate totals array directly from the response object
            const totalsArray = json[`rao_dev_ob_${config.identifier}_totals`];
            const totalEntry = totalsArray?.find(
                obTotal => obTotal.rao_dev_att_id === attrId
            );
            console.log("Total Array: ",totalsArray);
            console.log("Total Entry: ",totalEntry);
