...
try {
    // Validate rao_dev_id
    if (!isset($_GET['rao_dev_id']) || empty($_GET['rao_dev_id'])) {
        throw new Exception("Missing or invalid rao_dev_id");
    }
    $rao_dev_id = intval($_GET['rao_dev_id']);

    // Fetch attributes from tb_rao_dev_attributes
    $sql_fetch_attr = "SELECT rao_dev_att_id, attribute_name 
                       FROM tb_rao_dev_attributes 
                       WHERE isDisplayed = 1 AND rao_dev_id = ? 
                       ORDER BY rao_dev_att_id ASC";
    
    $stmt = mysqli_prepare($con, $sql_fetch_attr);
    mysqli_stmt_bind_param($stmt, "i", $rao_dev_id);
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $attributes[] = [
            'rao_dev_att_id' => $row['rao_dev_att_id'],
    ...
    // Check attribute values in tb_rao_dev_ap_data and tb_rao_dev_ob_data
    foreach ($attributes as &$attribute) {
        // Check tb_rao_dev_ap_data for non-zero attribute values
        $sql_fetch_ap_data = "SELECT attribute_value 
                              FROM tb_rao_dev_ap_data 
                              WHERE rao_dev_att_id = ? AND attribute_name = ?";
        $stmt_ap = mysqli_prepare($con, $sql_fetch_ap_data);
        mysqli_stmt_bind_param($stmt_ap, "is", $attribute['rao_dev_att_id'], $attribute['attribute_name']);
        ...
        // Check tb_rao_dev_ob_data for non-zero attribute values
        $sql_fetch_ob_data = "SELECT attribute_value 
                              FROM tb_rao_dev_ob_data 
                              WHERE rao_dev_att_id = ? AND attribute_name = ?";
        $stmt_ob = mysqli_prepare($con, $sql_fetch_ob_data);
        mysqli_stmt_bind_param($stmt_ob, "is", $attribute['rao_dev_att_id'], $attribute['attribute_name']);
