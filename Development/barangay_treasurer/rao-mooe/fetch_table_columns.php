...
try {
    // Validate rao_mooe_id
    if (!isset($_GET['rao_mooe_id']) || empty($_GET['rao_mooe_id'])) {
        throw new Exception("Missing or invalid rao_mooe_id");
    }
    $rao_mooe_id = intval($_GET['rao_mooe_id']);

    // Fetch attributes from tb_rao_mooe_attributes
    $sql_fetch_attr = "SELECT rao_mooe_att_id, attribute_name 
                       FROM tb_rao_mooe_attributes 
                       WHERE isDisplayed = 1 AND rao_mooe_id = ? 
                       ORDER BY rao_mooe_att_id ASC";
    
    $stmt = mysqli_prepare($con, $sql_fetch_attr);
    mysqli_stmt_bind_param($stmt, "i", $rao_mooe_id);
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $attributes[] = [
            'rao_mooe_att_id' => $row['rao_mooe_att_id'],
    ...
    // Check attribute values in tb_rao_mooe_ap_data and tb_rao_mooe_ob_data
    foreach ($attributes as &$attribute) {
        // Check tb_rao_mooe_ap_data for non-zero attribute values
        $sql_fetch_ap_data = "SELECT attribute_value 
                              FROM tb_rao_mooe_ap_data 
                              WHERE rao_mooe_att_id = ? AND attribute_name = ?";
        $stmt_ap = mysqli_prepare($con, $sql_fetch_ap_data);
        mysqli_stmt_bind_param($stmt_ap, "is", $attribute['rao_mooe_att_id'], $attribute['attribute_name']);
        ...
        // Check tb_rao_mooe_ob_data for non-zero attribute values
        $sql_fetch_ob_data = "SELECT attribute_value 
                              FROM tb_rao_mooe_ob_data 
                              WHERE rao_mooe_att_id = ? AND attribute_name = ?";
        $stmt_ob = mysqli_prepare($con, $sql_fetch_ob_data);
        mysqli_stmt_bind_param($stmt_ob, "is", $attribute['rao_mooe_att_id'], $attribute['attribute_name']);
